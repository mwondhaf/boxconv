...
export function VendorSectionCards({ data = defaultData }: VendorSectionCardsProps) {
  return (
    <div className="*:data-[slot=card]:from-primary/5 *:data-[slot=card]:to-card dark:*:data-[slot=card]:bg-card grid grid-cols-1 gap-4 px-4 *:data-[slot=card]:bg-gradient-to-t *:data-[slot=card]:shadow-xs lg:px-6 @xl/main:grid-cols-2 @5xl/main:grid-cols-4">
        ...
        <CardHeader>
          ...
          <CardAction>
            <Badge variant="outline">
              ...
              {data.totalRevenue.isPositive ? "+" : ""}{data.totalRevenue.trend}%
            </Badge>
          </CardAction>
        </CardHeader>
        <CardFooter className="flex-col items-start gap-1.5 text-sm">
          <div className="line-clamp-1 flex gap-2 font-medium">
            {data.totalRevenue.isPositive ? "Trending up" : "Trending down"} this month
            {data.totalRevenue.isPositive ? (
      ...
      <Card className="@container/card">
        ...
        <CardFooter className="flex-col items-start gap-1.5 text-sm">
          <div className="line-clamp-1 flex gap-2 font-medium">
            {data.totalOrders.isPositive ? "Orders increasing" : "Orders decreasing"}
            {data.totalOrders.isPositive ? (
              <IconTrendingUp className="size-4 text-green-600 dark:text-green-400" />
            ) : (
              <IconTrendingDown className="size-4 text-red-600 dark:text-red-400" />
        ...
        <CardHeader>
          ...
          <CardAction>
            <Badge variant="outline">
              ...
              {data.activeProducts.isPositive ? "+" : ""}{data.activeProducts.trend}%
            </Badge>
          </CardAction>
        </CardHeader>
        <CardFooter className="flex-col items-start gap-1.5 text-sm">
        ...
        <CardHeader>
          ...
          <CardAction>
            <Badge variant="outline">
              ...
              {data.totalCustomers.isPositive ? "+" : ""}{data.totalCustomers.trend}%
            </Badge>
          </CardAction>
        </CardHeader>
        <CardFooter className="flex-col items-start gap-1.5 text-sm">
          <div className="line-clamp-1 flex gap-2 font-medium">
            {data.totalCustomers.isPositive ? "Customer base growing" : "Customer retention needed"}
            {data.totalCustomers.isPositive ? (
              <IconTrendingUp className="size-4 text-green-600 dark:text-green-400" />
            ) : (
              <IconTrendingDown className="size-4 text-red-600 dark:text-red-400" />
